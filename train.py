...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        ckpt = torch_load_checkpoint(weights, map_location='cpu')  # load checkpoint to CPU to avoid CUDA memory leak
    ...
    amp = False if opt.no_amp else check_amp(model)  # check AMP
    ...
    scaler = smart_grad_scaler(device.type, enabled=amp)
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                strip_optimizer(f, half=amp)  # strip optimizers
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:
        ...
        if opt_yaml.is_file():
            ...
            d = torch_load_checkpoint(last, map_location='cpu')['opt']
